...
void loadJSON(Add *info) {
    ...
    const char* properties[] = { "id", "name", "age", "gender", "height", "weight"};
    char** infovar[] = {&info->id,&info->name,&info->age,&info->gender,&info->height,&info->weight};
...
int main(void) {
  ...
  while (FCGI_Accept() >= 0) {
   ...
   loadJSON(&info);
    ...
    dbres = sqlite3_open_v2("data.db", &db, SQLITE_OPEN_READWRITE,NULL);
