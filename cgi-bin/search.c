...
//gcc -o search.o search.c -lsqlite3 -ljansson  -lfcgi
...
int main() {
 while (FCGI_Accept() >= 0) {
    ...
    const char *sql = "SELECT * FROM info WHERE id LIKE ?";
    ...
    while ((rc = sqlite3_step(stmt)) == SQLITE_ROW) {
            ...
            json_object_set_new(object, "age", json_string((const char*)sqlite3_column_text(stmt, 2)));
...
//curl -X POST -H "Content-Type: application/json" -d '[{"name": "uid", "value": "21100001109"}]' http://172.28.98.30:8080/test/test/cgi-bin/search.o
