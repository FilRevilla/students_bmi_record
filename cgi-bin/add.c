...
//gcc -o add.o add.c -lsqlite3 -ljansson  -lfcgi
...
void create_json_array(Add info) {
    ...
    json_object_set_new(obj, "height", json_string(info.height));
    json_object_set_new(obj, "weight", json_string(info.weight));
    json_object_set_new(obj, "bmi", json_string(info.bmi));
    json_object_set_new(obj, "weight_status", json_string(info.weight_status));
...
void loadJSON(Add *info) {  
    ...
    const char* properties[] = { "id", "name", "age", "gender", "height", "weight", "bmi", "weight_status" };
    char** infovar[] = {&info->id,&info->name,&info->age,&info->gender,&info->height,&info->weight,&info->bmi,&info->weight_status};
...
int main(void) {
  ...
  while (FCGI_Accept() >= 0) {
   ...
   loadJSON(&info);
    ...
    dbres |= sqlite3_bind_text(stmt, 3, info.age, -1, SQLITE_TRANSIENT);
    dbres |= sqlite3_bind_text(stmt, 4, info.gender, -1, SQLITE_TRANSIENT);
    dbres |= sqlite3_bind_text(stmt, 5, info.height, -1, SQLITE_TRANSIENT);
    dbres |= sqlite3_bind_text(stmt, 6, info.weight, -1, SQLITE_TRANSIENT);
    dbres |= sqlite3_bind_text(stmt, 7, info.bmi, -1, SQLITE_TRANSIENT);
    dbres |= sqlite3_bind_text(stmt, 8, info.weight_status, -1, SQLITE_TRANSIENT);   
